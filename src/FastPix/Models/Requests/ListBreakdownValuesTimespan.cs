...
namespace fastpix.io.Models.Requests
{
    ...
    public static class ListBreakdownValuesTimespanExtension
    {
        public static string Value(this ListBreakdownValuesTimespan value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ListBreakdownValuesTimespan ToEnum(this string value)
        {
            foreach(var field in typeof(ListBreakdownValuesTimespan).GetFields())
            {
                ...
                {
                    return (ListBreakdownValuesTimespan)field.GetValue(null);
                }
            }
