...
namespace fastpix.io.Models.Requests
{
    ...
    public static class DimensionsIdExtension
    {
        public static string Value(this DimensionsId value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static DimensionsId ToEnum(this string value)
        {
            foreach(var field in typeof(DimensionsId).GetFields())
            {
                ...
                {
                    return (DimensionsId)field.GetValue(null);
                }
            }
