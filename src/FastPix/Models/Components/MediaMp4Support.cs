...
namespace fastpix.io.Models.Components
{
    ...
    public static class MediaMp4SupportExtension
    {
        public static string Value(this MediaMp4Support value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static MediaMp4Support ToEnum(this string value)
        {
            foreach(var field in typeof(MediaMp4Support).GetFields())
            {
                ...
                {
                    return (MediaMp4Support)field.GetValue(null);
                }
            }

            throw new Exception($"Unknown value {value} for enum MediaMp4Support");
        }
    }
